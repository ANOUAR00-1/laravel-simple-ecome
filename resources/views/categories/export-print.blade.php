...
<head>
    ...
    <title>Export Catégories - {{ now()->format('d/m/Y') }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="no-print" style="margin-bottom: 20px;">
        ...
        <a href="{{ route('categories.index', $filters ?? []) }}"
    ...
    <div class="header">
        ...
        <div class="export-info">
            ...
            <p><strong>Total:</strong> {{ $categories->count() }} catégorie(s)</p>
        </div>
    </div>
        ...
        <tbody>
            @foreach($categories as $category)
                <tr>
                    ...
                    <td class="text-right">{{ $category->produits_count ?? 0 }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div class="summary">
        <h3>Résumé</h3>
        <p><strong>Total Catégories:</strong> {{ $categories->count() }}</p>
        <p><strong>Total Produits:</strong> {{ $categories->sum('produits_count') }}</p>
    </div>
